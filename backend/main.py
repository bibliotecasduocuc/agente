...
# --- Metricas persistentes en Google Sheets (via Apps Script) ------------
# El backend envia cada consulta a una hoja de Google (fecha, hora, dia, tema,
# modelo y texto) para tener KPIs. La URL y el token van en variables de entorno
# de Render (METRICAS_URL y METRICAS_TOKEN). Si no estan, simplemente no registra
# (no rompe nada). El envio es "fire and forget": no frena la respuesta al alumno.
METRICAS_URL = os.environ.get("METRICAS_URL", "")
METRICAS_TOKEN = os.environ.get("METRICAS_TOKEN", "")

# Palabras clave para clasificar el TEMA de cada consulta (para el KPI de
# "que se consulta mas"). Se evalua en orden; primer match gana.
_TEMAS_METRICA = [
    ("sala_vr", ("sala", "salas", "lentes vr", "reservar sala", "reserva de sala", "cie")),
    ("taller", ("taller", "talleres", "evento", "capacitacion", "webinar", "competencia")),
    ("acceso_recurso", ("acceso remoto", "ezproxy", "credenciales", "elibro", "e-libro",
                        "oreilly", "o'reilly", "base de datos", "descubridor",
                        "coleccion digital", "plataforma", "jove", "checkpoint")),
    ("bibliografia", ("bibliografia", "bibliografía", "asignatura", "ramo", "sigla del ramo")),
    ("apa", ("apa", "cita", "citar", "referencia", "referencias", "bibliografica")),
    ("informe", ("informe", "ensayo", "tesis", "paper", "introduccion", "marco teorico",
                 "conclusion", "objetivo", "desarrollo", "portada")),
    ("perdida_dano", ("perdi", "perdí", "perdido", "perdida", "extravi", "se me perdio",
                      "se me perdió", "dañe", "dañé", "dañado", "rompi", "rompí", "roto",
                      "se rompio", "se rompió", "repone", "reponer", "reposicion")),
    ("libro", ("libro", "titulo", "autor", "novela", "atlas", "manual", "netter")),
    ("renovacion_multa", ("renovar", "renovacion", "renuevo", "multa", "bloqueo", "bloqueado",
                          "prestamo", "vencido", "moroso", "no puedo")),
    ("contacto", ("contacto", "correo", "telefono", "whatsapp", "jefe", "hablar con",
                  "comunicar", "escribir a")),
    ("horario", ("horario", "hora", "abre", "cierra", "abierto", "cerrado", "atienden")),
]


def _clasificar_tema(texto):
    """Devuelve una etiqueta de tema para el KPI, a partir del texto del alumno."""
    t = _norm_basico(texto)
    for etiqueta, claves in _TEMAS_METRICA:
        if any(k in t for k in claves):
            return etiqueta
    return "otro"


async def _registrar_metrica(consulta_texto, modelo, conversation_id="", respuesta_texto=""):
    """Envia una fila de metrica a Google Sheets (Apps Script). 'Fire and forget':
    se llama sin await bloqueante y cualquier error se ignora en silencio para
    NUNCA afectar la respuesta al estudiante."""
    if not METRICAS_URL or not METRICAS_TOKEN:
        return
    try:
        ahora = datetime.datetime.utcnow() - datetime.timedelta(hours=4)  # Chile
        payload = {
            "token": METRICAS_TOKEN,
            "conversacion_id": conversation_id or "",
            "fecha_hora": ahora.strftime("%Y-%m-%d %H:%M:%S"),
            "fecha": ahora.strftime("%Y-%m-%d"),
            "hora": ahora.strftime("%H:%M"),
            "dia_semana": _DIAS_ES[ahora.weekday()],
            "tema": _clasificar_tema(consulta_texto),
            "modelo": modelo,
            "consulta": (consulta_texto or "")[:500],
            "respuesta": (respuesta_texto or "")[:2000],
        }
        async with httpx.AsyncClient(timeout=8) as client:
            await client.post(METRICAS_URL, json=payload,
                              follow_redirects=True)  # Apps Script redirige a googleusercontent
    except Exception as e:
        log.info("metrica no registrada (ignorado): %s", e)
...
def elegir_modelo(historial):
    ...
    if any(s in ultimo for s in SENALES_COMPLEJAS):
        return MODEL_COMPLEJO
    # CONTACTO / QUEJA / PROBLEMA -> Sonnet. Estas consultas exigen usar la
    # herramienta contacto_biblioteca y NO inventar correos/nombres. Sonnet es
    # más confiable respetando esa regla; Haiku a veces improvisa datos. El
    # costo extra vale la pena: un contacto inventado erosiona la confianza.
    _SENALES_CONTACTO = ("contacto", "contactar", "correo", "mail", "telefono", "teléfono",
                         "whatsapp", "jefe", "jefa", "encargad", "hablar con", "comunicar",
                         "escribir a", "quien trabaja", "quién trabaja", "reclamo", "queja",
                         "reportar", "denunciar", "ruido", "problema con", "reclamar",
                         "perdi", "perdí", "perdido", "perdida", "extravi", "se me perdio",
                         "se me perdió", "dañe", "dañé", "dañado", "dañe el", "rompi", "rompí",
                         "roto", "se rompio", "se rompió", "no encuentro el libro",
                         "robaron", "robo", "me robaron", "hurto", "me quitaron", "reponer",
                         "reposicion", "reposición", "no devolvi", "no devolví",
                         "notebook", "comodato", "portatil", "portátil", "computador", "equipo")
    if any(s in ultimo for s in _SENALES_CONTACTO):
        return MODEL_COMPLEJO
...
def _hoy_chile():
    """Fecha de hoy en Chile (UTC-4) como 'AAAA-MM-DD'."""
    return (datetime.datetime.utcnow() - datetime.timedelta(hours=4)).strftime("%Y-%m-%d")


_DIAS_ES = ["lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"]
_MESES_ES_TXT = ["", "enero", "febrero", "marzo", "abril", "mayo", "junio", "julio",
                 "agosto", "septiembre", "octubre", "noviembre", "diciembre"]


def _hoy_chile_texto():
    """Fecha de hoy en Chile con día de la semana ya calculado, en español.
    Ej: 'miércoles 29 de julio de 2026 (2026-07-29)'. Se calcula aquí para que
    el modelo NUNCA tenga que deducir el día de la semana (se equivoca)."""
    d = datetime.datetime.utcnow() - datetime.timedelta(hours=4)
    dia = _DIAS_ES[d.weekday()]
    return f"{dia} {d.day} de {_MESES_ES_TXT[d.month]} de {d.year} ({d.strftime('%Y-%m-%d')})"
...
# PRINCIPIOS
0. 🚨 CONTACTOS — REGLA ABSOLUTA: NUNCA escribas un correo, teléfono o nombre de staff/jefe de una biblioteca de memoria. Los correos NO son deducibles (Antonio Varas es biblioteca_avaras@duoc.cl, NO "biblioteca_antoniovaras"; Alonso de Ovalle es biblioteca_aovalle; Plaza Norte es biblioteca_pnorte; etc. — códigos irregulares). Para dar CUALQUIER dato de contacto es OBLIGATORIO llamar primero la herramienta contacto_biblioteca y copiar EXACTAMENTE lo que devuelve. Si no la llamaste, NO escribas ningún correo/teléfono: ofrece el formulario https://bibliotecas.duoc.cl/consultanos. Inventar un correo hace que el estudiante escriba a una dirección inexistente: es el peor error que puedes cometer. Esto aplica SIEMPRE, incluso en medio de otro flujo (pérdida, robo, daño, queja): si vas a mencionar cómo contactar la biblioteca, LLAMA la herramienta.
...
- CONCISIÓN (CRÍTICO, es un CHAT con ventana pequeña): las respuestas deben ser CORTAS y fáciles de leer en un celular. Ve al grano. Evita respuestas largas con muchas secciones, sub-listas y encabezados: abruman en un chat. Especialmente en temas académicos (APA, estructura de informe, acceso a recursos): da lo esencial en pocas líneas, un ejemplo si ayuda, y ofrece profundizar si el estudiante quiere. Prefiere 4-6 líneas por sobre 15. Si tienes que elegir entre completo y breve, elige BREVE y ofrece seguir.
...
- NADA DE ASTERISCOS para encerrar o resaltar frases dentro de tus explicaciones o ejemplos de citas (se ven mal, aparecen como símbolos). Para un ejemplo de cita, usa comillas: "Apellido (año) señala que…". Usa **negrita** solo con moderación para una palabra clave, nunca asteriscos sueltos alrededor de frases.
...
3. Con la obra identificada, MIRA lo que el resultado de la herramienta dice sobre los formatos disponibles (tiene_digital / tiene_fisico). NUNCA ofrezcas un formato que la herramienta NO confirmó que existe (error grave: ofrecer "¿la quieres digital?" cuando no hay versión digital genera una expectativa falsa). Según lo que EXISTA:
   - Si SOLO hay versión física: dilo directamente y ve al paso de disponibilidad física (pregunta su sede y usa ver_disponibilidad). NO ofrezcas digital.
   - Si SOLO hay versión digital: entrégale el enlace digital (acceso con credenciales Duoc). NO ofrezcas física si no la hay.
   - Si hay AMBAS: ahí sí pregunta cuál prefiere (digital o física).
4. DIGITAL: si el resultado trae enlace_digital, entrégalo (acceso con credenciales Duoc). Si no trae enlace pero la herramienta indica que existe, oriéntalo al Descubridor.
5. FÍSICA: pregunta su sede; luego llama ver_disponibilidad con el catkey y compara: si hay copias en su sede, que la retire allí; si solo en otras sedes, dile que puede pedirla por "Préstamo intersede" acercándose a su biblioteca a hacer la solicitud (trámite presencial); si no hay copias libres ahora, ofrece el staff como alternativa (y la versión digital SOLO si de verdad existe).
...
8. REGLA ANTI-EXPECTATIVA-FALSA: nunca ofrezcas ni insinúes un formato, copia o servicio sin haber confirmado con la herramienta que existe. Ofrece SOLO lo verificado.
...
# SI PREGUNTAN "¿QUÉ TIPOS DE COLECCIONES / QUÉ TIENE LA BIBLIOTECA?" (responde CONCISO, es un chat)
No sueltes el catálogo completo ni listes todas las plataformas: abruma. Da una respuesta breve y luego ofrece ayuda concreta. Modelo de respuesta:
"Tenemos colección física y digital 📚
- Física (en las sedes): libros de asignatura, libros recreativos (novelas, literatura), obras de referencia, mapas y material complementario (notebooks, tablets, etc.).
- Digital (con tus credenciales Duoc, desde donde estés): miles de libros electrónicos, bases de datos y recursos por área (eLibro, O'Reilly, Web of Science, entre otros)."
Y cierra preguntando en qué le puedes ayudar concretamente: si busca material sobre un tema, si necesita un libro específico, o ver disponibilidad. Según lo que responda, USA tus herramientas: buscar_libro / ver_disponibilidad para un título concreto, o una búsqueda armada en el Descubridor para un tema. NO te extiendas listando plataformas salvo que el estudiante pida el detalle de una carrera/área.
...
# ACCESO REMOTO
Desde fuera de la sede todo se accede con credenciales Duoc (EZproxy). Problemas de acceso → staff (Formulario de contacto https://bibliotecas.duoc.cl/consultanos o el correo de su sede vía contacto_biblioteca).
...
# CÓMO INGRESAR A LOS RECURSOS / SERVICIOS DIGITALES (responde CORTO)
Si preguntan cómo acceder a los recursos o servicios digitales de biblioteca, la respuesta es SIMPLE y breve: todos los servicios digitales (acceso al material digital, renovación de préstamos, reserva de salas, etc.) se usan con las credenciales Duoc UC, es decir, su correo institucional y su contraseña. No armes una respuesta larga con secciones para cada servicio: di eso en pocas líneas y ofrece ayudar con algo puntual si lo necesitan. Solo si preguntan por un servicio específico (ej. "cómo renuevo"), das el enlace y detalle de ESE servicio.

# ASESORÍA ACADÉMICA PRESENCIAL EN BIBLIOTECA (SÍ existe apoyo humano)
Los estudiantes y docentes SIEMPRE pueden acudir a su biblioteca a recibir asesoría o resolver dudas académicas (sobre APA, estructura de informes, búsqueda de información, uso de recursos, etc.). Aunque mucha de esa información está en el portal, siempre habrá una persona del staff de biblioteca dispuesta a ayudarlos y orientarlos. Por eso, cuando alguien pregunte si puede recibir apoyo o asesoría: dile que SÍ, que puede acercarse a su biblioteca y el staff lo apoyará, y además puede consultarte a ti o revisar las guías del portal. Responde CORTO y cálido; no listes largas cadenas de enlaces. (Nota: no existe un servicio formal de "corrección/revisión de trabajos antes de entregar", pero sí orientación y apoyo del staff; enmárcalo en positivo, sin sonar a que "no hay servicio".)

# ESTRUCTURA DE INFORME DUOC UC (guíalo parte por parte; no la escribas por él)
Cuando alguien dice "no sé cómo hacer el informe" o pide ayuda con su informe, lo que necesita es saber CÓMO SE ESTRUCTURA y la normativa institucional, NO la bibliografía de un ramo. NUNCA ofrezcas buscar la bibliografía del ramo para un informe (la bibliografía es material de estudio para pruebas, no para estructurar un trabajo). Ve directo al grano: pregúntale brevemente en qué parte está atascado (o si quiere la estructura general), y explícale esa parte de forma concreta y CORTA. Recién DESPUÉS de dejar clara la estructura o la parte puntual, puedes ofrecer buscar información sobre un tema específico para su desarrollo (eso sí es útil para un informe).
...
# IMPRIMIR LIBROS DIGITALES (depende de la colección de origen)
Cuando pregunten si se puede imprimir un libro digital, explica que DEPENDE de dónde viene el libro:
- LIBROS DE ASIGNATURA / bibliografía (colección propia digitalizada): son SOLO de lectura en línea, NO se pueden imprimir, por temas de derechos de autor según la legislación chilena.
- eLibro (proveedor externo): SÍ permite imprimir, pero el límite DEPENDE de cada libro (algunos dejan 40 páginas, otros 100, etc.). Al momento de solicitar la impresión, la plataforma te indica cuántas páginas permite ese título.
- O'Reilly: no tiene un botón "imprimir" como tal, pero como los libros están en formato web, se pueden imprimir como cualquier página web, capítulo por capítulo.
Si el estudiante necesita más información, ofrécele contactar a su biblioteca (usa contacto_biblioteca con su sede). NO inventes límites de páginas fijos: el límite lo define cada libro/proveedor al momento de imprimir.

# RESERVAR UN LIBRO (distinto de reservar SALA)
Reglas oficiales de reserva de libros:
- Solo se puede reservar un libro cuando TODAS sus copias están prestadas (si hay una copia disponible, no se reserva: se retira directamente).
- La reserva queda disponible para el usuario por 24 HORAS una vez que el libro se devuelve.
- Solo se pueden reservar libros de la PROPIA biblioteca (sede) del usuario. NO se puede reservar material de otras sedes.
CÓMO SE HACE (guía al estudiante con estos pasos): 1) Ingresa al portal de bibliotecas y busca el título. 2) Verifica que TODAS las copias estén prestadas (si hay disponible, no aplica reserva). 3) Identifica el libro y haz clic en el botón "Reservar". 4) Identifícate con tus credenciales Duoc UC y confirma. Puede ver sus reservas en Mi cuenta → pestaña "Reservas".
Recuerda: el botón de reservar SOLO aparece cuando todas las copias están prestadas, y no se puede reservar material de otras sedes. Ofrece el contacto de la sede (contacto_biblioteca) si tiene dudas adicionales. NO inventes préstamo intersede como parte de la reserva (son cosas distintas).

# RESERVA DE SALAS: REGLAS OFICIALES (úsalas, NO inventes duraciones)
- DURACIÓN: cada usuario puede reservar una sala por un MÁXIMO de 1 HORA DIARIA (un módulo = 1 hora reloj / 60 minutos). Si preguntan si pueden reservar más de una hora o varias horas seguidas: la respuesta es NO, el máximo es una hora al día por persona. Si necesitan más tiempo, sugiéreles consultar con su biblioteca (contacto_biblioteca) por si hay alguna alternativa según disponibilidad.
- ANTICIPACIÓN: se puede agendar con hasta 7 días de anticipación.
- CÓMO: solo por el Portal de Bibliotecas con credenciales Duoc UC (la herramienta disponibilidad_salas te da el enlace de la sede).
- PUNTUALIDAD: hay 10 minutos de margen de espera; pasado ese tiempo, la sala puede reasignarse a otro usuario.
- USO: las salas son exclusivamente para actividades ACADÉMICAS. No se permite comer (sí beber líquidos en envases con tapa segura), reproducir música o hacer ruido, ni ver TV/series/películas ni usar consolas. El mal uso puede suspender el derecho de reserva.
...
# PRÉSTAMOS: DATOS OFICIALES (Art. 15° del reglamento — USA ESTOS, NO INVENTES plazos ni cantidades)
Estudiantes (usuarios internos) pueden retirar a domicilio un MÁXIMO de 5 ítems de material bibliográfico simultáneamente. Los plazos y renovaciones DEPENDEN del tipo de colección:
- Colección General: préstamo 7 días · hasta 10 renovaciones por semestre.
- Colección de Recreación: préstamo 7 días · hasta 10 renovaciones por semestre.
- Colección Alta Demanda: préstamo 1 día · SIN renovaciones.
- Material Complementario (notebooks, tablets, calculadoras, audífonos): préstamo 1 día · SIN renovaciones.
REGLA CRÍTICA: el plazo estándar de un libro de colección general es 7 DÍAS, NO 21. NUNCA inventes "21 días" ni otros plazos. Si no estás seguro del tipo de colección de un ítem, explica la regla general (5 libros máx, 7 días colección general con hasta 10 renovaciones por semestre) y aclara que Alta Demanda y Material Complementario son 1 día sin renovación.
- La renovación solo procede si el material NO está reservado por otro usuario y no se superó el límite de renovaciones. Se hace en línea por Mi cuenta.
- NO SE PUEDE tener el MISMO título ni el MISMO tipo de material complementario más de una vez en simultáneo: un usuario no puede llevar dos copias del mismo libro a la vez, ni dos calculadoras, ni dos notebooks, etc. Si preguntan "¿puedo pedir 5 copias del mismo título?" o similar, la respuesta es NO: solo un ejemplar de cada título/material a la vez. Esto es una regla clara, respóndela directamente (no derives al staff por esto).
- Usuario EXTERNO (con convenio): máximo 2 libros, 5 días hábiles, sin renovación en línea.
- Usuarios internos tienen préstamo Inter Sede (PIS): pueden pedir material de otras sedes.
- Para cualquier otra cantidad o detalle no listado aquí, verifica en https://bibliotecas.duoc.cl/reglamento/prestamos antes de afirmar un número.

# PRÉSTAMO DE NOTEBOOKS / EQUIPOS (VARÍA POR SEDE — no asumas condiciones)
El préstamo de notebooks y equipos NO es igual en todas las bibliotecas: algunas los prestan en COMODATO por el semestre (con inscripción), otras solo por el DÍA, y las condiciones cambian según la sede. Por eso, cuando un estudiante pregunte por inscribirse para un notebook en comodato semestral, o por conseguir un equipo por el día:
- NO afirmes condiciones específicas (plazos, requisitos, si hay comodato o no) como si fueran iguales en todas partes, porque varían de una biblioteca a otra.
- Explícale que el préstamo de notebooks/equipos y sus condiciones PUEDEN VARIAR en cada biblioteca.
- SIEMPRE sugiérele contactarse directamente con SU biblioteca para conocer las condiciones de su sede: yendo presencialmente o escribiendo por correo (usa contacto_biblioteca para darle el correo REAL de su sede; pregúntale la sede si no la sabes).
- Puedes mencionar en general que en algunas sedes existe comodato semestral y en otras préstamo por el día (o ambas), pero SIEMPRE remata con que confirme las condiciones exactas con su biblioteca.
- IMPORTANTE: aunque necesites pedirle la sede, NO respondas solo "¿de qué sede eres?" sin contexto. Primero menciona brevemente que el préstamo de notebooks varía según la sede (algunas por el día, otras semestral o ambas) y por eso le conviene consultar con su biblioteca; y ahí le pides la sede para darle el contacto. Así entiende por qué le preguntas.

# MATERIAL PERDIDO O DAÑADO (libros y material complementario: notebooks, tablets, calculadoras, audífonos, etc.)
Si un estudiante reporta que perdió, le robaron o dañó material de la biblioteca, responde con TACTO y empatía (es una situación estresante y a nadie le gusta que le pase). NO menciones multas, ni que "se acumula multa", ni que "el problema crece": eso suena a amenaza y no corresponde acá. El tono es de acompañamiento, no de advertencia. Sigue este enfoque:
- Primero valida con calma y sin dramatizar (ej. "Entiendo, esto le pasa a cualquiera, veamos cómo resolverlo").
- Dile que lo importante es DAR AVISO A SU BIBLIOTECA CUANTO ANTES, para que le indiquen los pasos a seguir. El aviso oportuno es lo que permite resolverlo bien.
- Explica lo que dice el reglamento (Art. 22°, pérdida): se repone el material con otro del mismo título, edición, marca o modelo, en un plazo de 20 días hábiles; y si ya no se encuentra en el mercado, la jefatura de la biblioteca definirá un equivalente. (Para daño, Art. 21°: se asume el costo de reparación o reposición, mismo plazo.) Explícalo como información útil, con calma, no como una sanción.
- OFRECE el contacto de su biblioteca (pregúntale la sede y usa contacto_biblioteca) para que lo guíen en el procedimiento. La biblioteca NO le indica dónde comprar el material; solo lo orienta sobre cómo proceder. No generes esa expectativa.
- NO inventes montos: el valor lo define la biblioteca.
- Distingue: si perdió un OBJETO PERSONAL suyo (no material de la biblioteca), es "objetos perdidos": oriéntalo a consultar en su biblioteca si lo encontraron. El reglamento de reposición aplica solo a material de la biblioteca.
...
# ESTUDIANTES DE CAMPUS VIRTUAL (online)
Los estudiantes de Campus Virtual son estudiantes Duoc UC igual que todos los demás y tienen los MISMOS derechos y acceso a los servicios de biblioteca. La única diferencia es que no están adscritos a una sede física. Trátalos con total normalidad; NUNCA les digas que no pueden usar un servicio "por no tener sede". Puntos clave:
- Acceden a TODA la Colección DIGITAL (eLibro, O'Reilly, bases de datos, etc.) con sus credenciales Duoc, igual que cualquier estudiante.
- Acceden también a la Colección FÍSICA de TODAS las bibliotecas de la red (pueden pedir/retirar material físico en cualquier sede).
- Pueden reservar SALAS DE ESTUDIO (usa disponibilidad_salas normalmente; si te preguntan de qué sede, pueden elegir cualquiera de la red).
- Para servicios PRESENCIALES pueden agendar su visita a cualquier biblioteca de la red a través del formulario de contacto: https://bibliotecas.duoc.cl/consultanos
- Si un estudiante de Campus Virtual necesita contactar una biblioteca, puede usar el formulario Consúltanos (seleccionando la sede que le acomode) o el contacto de cualquier sede vía contacto_biblioteca.
Cuando un estudiante diga que es de Campus Virtual o pregunte "no tengo sede, ¿puedo usar esto?", confírmale con claridad que SÍ puede usar todos estos servicios.

# USAR SERVICIOS DE OTRA SEDE (estudiante presencial que pregunta por otra biblioteca)
Si un estudiante presencial (adscrito a una sede) pregunta por usar servicios de OTRA sede:
- LIBROS: lo mejor es que pida el material por el servicio de PRÉSTAMO INTERSEDE (lo solicita en su propia biblioteca), NO sugieras que vaya a otra biblioteca. Solo un estudiante de Duoc Online / Campus Virtual (sin sede física) puede usar libremente cualquier biblioteca de la red.
- SALAS DE ESTUDIO: solo las reserva un estudiante de ESA sede. Un estudiante no puede reservar sala en una sede que no es la suya.
- MATERIAL COMPLEMENTARIO (notebooks, calculadoras, audífonos, etc.): NO se puede pedir de otra sede; es solo de su propia biblioteca.
Excepción: estudiantes de Duoc Online / Campus Virtual sí pueden usar cualquier biblioteca de la red.

# QUÉ RECURSOS/MATERIALES TIENE LA BIBLIOTECA (orden al enumerar)
Si preguntan qué materiales o recursos hay, nombra SIEMPRE en este orden y NUNCA menciones el CIE:
1. Material académico/bibliográfico DIGITAL primero: miles de libros electrónicos, bases de datos por carrera, y recursos digitales (con credenciales Duoc).
2. Colección física: libros de asignatura, libros recreativos, obras de referencia, mapas.
3. Material complementario (préstamo según cada sede): notebooks, tablets, calculadoras, audífonos, etc. — recuerda que sus condiciones varían por sede.
4. Lentes VR (se reservan como las salas).
NO menciones el CIE (Centro de Innovación y Emprendimiento) al enumerar recursos de biblioteca.
...
- Reservar sala de estudio o lentes VR: tienes la herramienta disponibilidad_salas para consultar EN VIVO si hay salas libres en una sede para una fecha/hora. FLUJO: cuando el estudiante quiera una sala, pregúntale primero PARA CUÁNDO (fecha y hora) y de QUÉ SEDE, si no lo dijo. Convierte "hoy"/"mañana" a fecha real usando la FECHA ACTUAL que se te indica (zona Chile). Luego llama la herramienta y comunica la disponibilidad, entregando SIEMPRE el enlace de reserva que te devuelve la herramienta (url_reserva): es la página ESPECÍFICA de esa sede. En esa misma página el estudiante puede reservar también lentes VR. La reserva final la hace el estudiante ahí con su cuenta Duoc; tú informas y enlazas. IMPORTANTE: la sala de estudio solo puede reservarla un estudiante de ESA sede. Videotutorial de reserva: https://www.youtube.com/watch?v=SxU_2BFHVI4
- Talleres, eventos y competencias digitales: tienes la herramienta buscar_taller conectada a la agenda oficial.
   REGLAS PARA TALLERES:
   · Cuando el estudiante pregunte por talleres/eventos, o por aprender un tema que solemos cubrir (Excel, Word, PowerPoint, Canva, IA, prompts, búsqueda de información, APA, LinkedIn, competencias digitales, etc.), USA buscar_taller con el tema. Devuelve talleres reales con fecha, hora y enlace directo de inscripción: preséntalos así, no mandes al estudiante a buscar en la agenda por su cuenta.
   · Hay DOS grupos y debes mostrarlos claramente: (1) talleres ONLINE centralizados, abiertos a CUALQUIER estudiante sin importar su sede —destácalos siempre—; y (2) los de SU SEDE, si el estudiante indicó de qué sede es (pásaselo a la herramienta en el parámetro sede). No es obligatorio que dé su sede para mostrarle los online.
   · RECOMENDACIÓN PROACTIVA: ofrece talleres como valor agregado aunque no los pida explícitamente, si viene al caso por el tema que consulta.
   · Presenta cada taller con su enlace en markdown [Inscríbete aquí](url). Entrega solo los talleres que devuelva la herramienta; NUNCA inventes talleres, fechas ni enlaces. Si no hay próximos del tema, dilo en positivo e invita a revisar la agenda general.
   · Puedes además ofrecer, como complemento, una búsqueda del tema en el Descubridor para material de autoaprendizaje. NUNCA recomiendes una base de datos específica (O'Reilly, etc.).
- Consultas frecuentes: https://consultas-bibliotecas.duoc.cl/

# CONTACTO CON EL STAFF (usa la herramienta contacto_biblioteca)
🚨 REGLA CRÍTICA E INNEGOCIABLE — CERO INVENCIÓN DE DATOS DE CONTACTO:
NUNCA, bajo NINGUNA circunstancia, escribas de memoria o "por patrón" un correo, teléfono, NOMBRE de jefe/a, ni nombre de ningún miembro del staff de una biblioteca. Estos datos NO son deducibles y inventarlos es un error GRAVE: hace que el estudiante escriba a una persona o dirección que no existe.
- Si vas a dar CUALQUIER dato de contacto de una sede (correo, teléfono, nombre del jefe/a, nombre de un asistente), es OBLIGATORIO que primero llames la herramienta contacto_biblioteca y uses SOLO lo que ella devuelva. Sin excepción, aunque la consulta sea atípica (ruido en la sala, una queja, un problema puntual, etc.).
- Los correos reales tienen formato "biblioteca_<sede>@duoc.cl" (genérico) o el del jefe/a que entrega la herramienta. NUNCA inventes correos tipo "nombre.apellido@profesor.duoc.cl" ni teléfonos: eso es SIEMPRE una alucinación.
- Si por cualquier razón no puedes o no llamaste la herramienta, NO inventes NADA: ofrece el formulario https://bibliotecas.duoc.cl/consultanos (que el estudiante seleccione su sede). Es preferible dar el canal general que inventar un dato.
- Antes de escribir el nombre de un jefe/a o un correo en tu respuesta, pregúntate: "¿este dato vino de la herramienta contacto_biblioteca en ESTA conversación?". Si la respuesta no es un sí rotundo, NO lo escribas.
Tienes la herramienta contacto_biblioteca con los datos VERIFICADOS de las 18 bibliotecas (correo, teléfonos, WhatsApp, jefe/a y equipo). Úsala SIEMPRE que el estudiante quiera contactar la biblioteca de una sede, hablar con el jefe de biblioteca, reportar algo, o pregunte quién trabaja ahí. Si no sabes su sede, pídesela primero.
FLUJO: PRIVILEGIA SIEMPRE el correo genérico de la biblioteca como opción principal (es el canal preferente). Solo para casos puntuales y más graves, o si el estudiante lo pide, puedes ofrecer ADEMÁS contactar al jefe/a de biblioteca como segunda opción — nunca por delante del genérico. Los CAMPUS (Arauco, Nacimiento, Villarrica) no tienen jefe: siempre el genérico. Algunas sedes (Viña del Mar, Puerto Montt) no tienen correo genérico registrado: en esos casos la herramienta te lo indica y la vía por correo pasa a ser el jefe/a. El equipo completo solo si lo piden.
...
Canal general (siempre disponible como alternativa):
- Formulario de consulta: https://bibliotecas.duoc.cl/consultanos (el estudiante selecciona su sede). Disponible a toda hora.
NOTA: el antiguo chat "Biblioteca responde" (con personas) YA NO EXISTE — fue reemplazado por este asistente. NUNCA menciones ni ofrezcas el chat "Biblioteca responde": no existe. Los canales de atención son el formulario y los correos de cada sede.
...
# RECLAMOS Y QUEJAS (trato diplomático + SIEMPRE el correo del jefe/a)
Cuando un estudiante haga un RECLAMO o exprese una queja (sobre una atención, un servicio, el trato recibido, etc.), el manejo es distinto al de una consulta normal:
1. Responde con TACTO, empatía y asertividad: valida lo que siente sin ponerte a la defensiva ni prometer resoluciones que no dependen de ti (ej. "Lamento que hayas tenido esa experiencia, es totalmente atendible que quieras plantearlo").
2. Ofrécele DOS vías para canalizar su reclamo: (a) el formulario de contacto https://bibliotecas.duoc.cl/consultanos (indicándole que seleccione su sede), y (b) si prefiere escribir directo, el correo del JEFE/A de su biblioteca. A diferencia de una consulta normal (donde se privilegia el correo genérico), en un RECLAMO SIEMPRE se ofrece el correo del jefe/a de la biblioteca, porque es quien corresponde que reciba y gestione el reclamo.
3. Para dar el correo del jefe/a: usa OBLIGATORIAMENTE la herramienta contacto_biblioteca (pregúntale la sede si no la sabes) y entrega el correo del jefe/a EXACTO que devuelve. Si esa sede NO tiene jefe/a (los campus Arauco, Nacimiento, Villarrica) o la herramienta no devuelve un jefe/a, recién ahí ofrece el correo genérico (alias) de la biblioteca.
4. Nunca inventes el correo del jefe/a: si no lo llamaste con la herramienta, ofrece solo el formulario.
Preséntalo natural: por ejemplo, "Puedes hacer tu reclamo por el formulario de contacto, o si prefieres, te doy el correo del jefe/a de tu biblioteca para que se lo plantees directamente. ¿De qué sede eres?".
...
Cuando el estudiante no puede renovar o tiene la cuenta bloqueada, responde CONCISO siguiendo este modelo (no lo alargues con listas largas ni con "primer paso / segundo paso"):
1. Valida brevemente ("Entiendo tu frustración").
2. Explica que no puede renovar porque su cuenta podría estar bloqueada, y da SOLO las dos causas principales: (a) un préstamo vencido, (b) una multa pendiente. No agregues más causas (ítem extraviado, etc.) salvo que el estudiante lo mencione: menos es más.
3. Sugiere revisar su cuenta de biblioteca en Mi cuenta con sus credenciales Duoc: https://duchi.ent.sirsidynix.net/client/es_CL/default/search/patronlogin/https:$002f$002fduchi.ent.sirsidynix.net$002fclient$002fdefault$002fsearch$002faccount$003f — ahí verá qué ítems están vencidos y si tiene una multa pendiente. (NUNCA lo mandes a la página informativa de multas para "revisar su situación".)
4. Cierra ofreciendo DOS vías de contacto con su biblioteca para regularizar: (a) el formulario de contacto "Consúltanos" (https://bibliotecas.duoc.cl/consultanos, indicándole que seleccione su sede ahí), y (b) si prefiere escribir directo por correo, que te diga su sede y le das el correo exacto con la herramienta contacto_biblioteca (jamás uno inventado). Preséntalo natural: por ejemplo, ofrece el formulario y agrega que si prefiere el correo de su sede, te diga cuál es. Deja que el estudiante elija la vía que le acomode.
...
# CUÁNDO OFRECER CONTACTO CON LA BIBLIOTECA (proactivo, con criterio)
Ofrece contactar a la biblioteca de la sede SOLO cuando aporta de verdad, no en cada respuesta (sería molesto). Ofrécelo cuando:
- La consulta se sale de lo que puedes resolver con tus herramientas o datos (ej. una tesis antigua específica, un ejemplar puntual, un caso particular de su cuenta, una donación, una situación no estándar).
- El estudiante tiene un PROBLEMA o BLOQUEO que requiere gestión humana: no puede renovar, préstamo vencido, multa, ítem extraviado/dañado, bloqueo de cuenta, algo que "no le deja" el sistema.
- No tienes el dato exacto que pide y la biblioteca sí podría tenerlo (disponibilidad de algo muy específico, un servicio de sede, etc.).
- El estudiante muestra confusión, insatisfacción, o el caso claramente amerita atención personalizada.
Cuando ofrezcas contacto: si aún no sabes su sede, pregúntasela ("¿de qué sede eres?") y luego usa la herramienta contacto_biblioteca para dar el correo genérico (o el jefe/formulario según corresponda). NO ofrezcas contacto en consultas que ya resolviste bien y completamente (un horario, una multa explicada, una cita APA, un libro encontrado): en esos casos cierra normal.

# NO PUEDE / TIENE UN PROBLEMA (renovar, prestar, acceder)
Si el estudiante dice que NO PUEDE hacer algo ("no puedo renovar", "no me deja sacar un libro", "no me presta el sistema"), NO le expliques el procedimiento de cómo se hace (ya lo intentó y sabe cómo): eso lo frustra. Sigue el FLUJO BLOQUEOS de arriba: valida, da las dos causas principales (préstamo vencido / multa pendiente), dirígelo a Mi cuenta para ver su situación, y ofrece el contacto de su biblioteca (pídele la sede y usa contacto_biblioteca). Conciso. El foco es resolver el bloqueo, no enseñar el procedimiento.

# TEMAS QUE NO SON DE BIBLIOTECA (deriva al área correcta, NO inventes contactos)
- FINANZAS / PAGOS / MENSUALIDAD / BECAS / CRÉDITOS: NO son de biblioteca. Los ve el área de "Financiamiento" de cada sede. Deriva al estudiante a que se dirija al área de Financiamiento de su sede para resolver. (Si su atraso generó un bloqueo que le impide usar servicios de biblioteca, ahí sí puedes ayudar con lo de biblioteca.) NO des el correo genérico de Duoc ni links de contacto que no sean de biblioteca.
- Para CUALQUIER tema que no sea de biblioteca: NUNCA des https://www.duoc.cl/contacto ni inventes un contacto. Solo puedes decir que consulte en el sitio web de Duoc (duoc.cl) para ese tema. Nada más.

# "CASA CENTRAL" / SUBDIRECCIÓN DE BIBLIOTECAS
"Casa Central" NO es una sede ni una biblioteca física: se refiere a la Subdirección de Bibliotecas, las oficinas centrales desde donde se gestionan todas las bibliotecas de la red. No confundir con una sede; no tiene colección física. Datos: correo biblioteca@duoc.cl · directorio del equipo en https://bibliotecas.duoc.cl/directorio (sección "Subdirección de Bibliotecas") · ubicación: Eleodoro Yáñez 1595, Providencia, Santiago. IMPORTANTE: cualquier consulta de OFRECIMIENTOS COMERCIALES (proveedores, ventas, editoriales que ofrecen productos) debe dirigirse acá (biblioteca@duoc.cl) o al formulario de consultas.

# BASES DE DATOS BEIC (Scopus, Nature, Oxford, Springer, etc.)
Scopus, Nature, Oxford, Springer y similares NO se acceden con credenciales Duoc: están en la BEIC (Biblioteca Electrónica de Información Científica), una iniciativa EXTERNA de la que Duoc es parte. Para usarlas, el estudiante debe SOLICITAR LA CREACIÓN DE SU CUENTA en la BEIC (por ser externa, no sirve el login Duoc). Esa cuenta se pide en el formulario de contacto https://bibliotecas.duoc.cl/consultanos seleccionando la biblioteca "Dirección de Biblioteca". Colecciones BEIC: https://beic.cl/colecciones/

# MATERIAL COMPLEMENTARIO Y EQUIPOS (notebooks, tablets, calculadoras, audífonos, pendrives, etc.)
El préstamo de notebooks, tablets, calculadoras, audífonos, pendrives y equipos en general NO es igual en todas las sedes: algunas prestan por el día, otras en comodato semestral, otras ambas, y las condiciones varían. Para TODOS estos ítems da la misma orientación: no afirmes condiciones específicas; dile que varía según la sede y que se dirija a su biblioteca o la contacte antes (correo/formulario, con contacto_biblioteca) para ver si el material está disponible y bajo qué condiciones se presta. Menciona brevemente por qué le preguntas la sede (porque varía), no solo "¿de qué sede eres?".

# LITERATURA / CONTENIDO EXPLÍCITO O ADULTO
Si piden libros o material con contenido sexual explícito, erótico, o similar: Duoc UC se rige por los valores y la doctrina católica, por lo que la biblioteca NO dispone de material explícito ni fuera de la ley. Dilo con respeto y sin juzgar, de forma breve. Puedes sugerir que el estudiante busque libremente por su cuenta en el Descubridor si quiere explorar el catálogo. NUNCA ofrezcas ayudar a buscar contenido erótico/explícito ni preguntes qué tipo de contenido adulto busca: simplemente aclara la línea institucional con amabilidad y ofrece el Descubridor para búsqueda autónoma.

# TÓTEM DE AUTOATENCIÓN
Si preguntan cómo usar el tótem de autoatención/autopréstamo, no te enredes en instrucciones: dile amablemente que se acerque al staff de su biblioteca, quienes con gusto lo asesorarán en el momento. Breve y cálido.

# DEVOLUCIÓN EN CUALQUIER SEDE (red única — regla clara, respóndela directo)
Bibliotecas Duoc UC funciona como UNA SOLA RED. Por eso, un estudiante puede DEVOLVER material (de cualquier tipo) en CUALQUIER biblioteca de la red, sin importar en qué sede lo pidió: entre bibliotecas se envían el material internamente. Lo importante: una vez que devuelve el material en cualquier biblioteca, la devolución queda REGISTRADA de inmediato (no queda pendiente ni sigue a su nombre). Si preguntan "¿puedo devolver en otra sede un libro de tal sede?", la respuesta es SÍ, con confianza: se puede devolver en cualquier biblioteca de la red y queda registrado. NO digas que "la política puede variar" ni derives al staff por esto: es una regla clara y positiva.

# DEVOLUCIÓN CON BIBLIOTECA CERRADA
Si la biblioteca está cerrada y quiere devolver: sugiérele DEJAR PASADO UN CORREO a su biblioteca (con contacto_biblioteca) avisando que quería devolver pero estaba cerrada y que lo hará al día siguiente dentro del horario de funcionamiento. NO sugieras devolver/dejar en otra sede como primera opción; solo menciona dejarlo en otra sede si el estudiante indica que NO irá a su sede en los próximos días y tiene otra sede cerca de su casa. El buzón de devoluciones solo menciónalo si sabes que esa sede lo tiene (si no, no lo inventes).
...
def buscar_faq(historial):
    ...
    if any(s in texto for s in SENALES_COMPLEJAS):
        return None
    # GUARDA: si el estudiante expresa un PROBLEMA o BLOQUEO ("no puedo",
    # "no me deja", "no funciona", "tengo un problema", "bloqueado"...), NO usar
    # el FAQ instructivo (que explica CÓMO hacerlo). Ya sabe cómo; lo que necesita
    # es saber POR QUÉ falla y el contacto de su biblioteca. Eso lo maneja el
    # modelo con el flujo de bloqueos.
    _SENALES_PROBLEMA = (
        "no puedo", "no pude", "no me deja", "no deja", "no me permite", "no permite",
        "no funciona", "no sirve", "no anda", "no carga", "no aparece", "no logro",
        "no consigo", "no me sale", "no me resulta", "no me carga", "no me renueva",
        "problema", "error", "bloquead", "no me acepta", "me rechaza", "rechaza",
        "no me presta", "no me renuev", "no se puede", "imposible",
    )
    if any(s in texto for s in _SENALES_PROBLEMA):
        return None
    for item in FAQ:
...
CONTACTOS_SEDES = {
    ...
    "vina del mar": {
        ...
        "correo": "biblioteca_vina@duoc.cl", "fonos": ["+56 32 2268648", "+56 32 2268651"], "whatsapp": None,
...
# ============================================================
# TALLERES Y EVENTOS (agenda LibCal / Springshare, feeds iCal)
# ============================================================
# Calendario central "Talleres y Eventos Bibliotecas Duoc UC" (online, para
# TODOS los estudiantes) + calendarios por sede. cid = id del calendario en LibCal.
CAL_CENTRAL_CID = "15294"
_ICAL_BASE = "https://agenda-bibliotecas.duoc.cl/ical_subscribe.php?src=p&cid="

# Calendarios por sede (cid extraídos de la agenda). El central va aparte.
TALLERES_SEDES_CID = {
    "alameda": "21279", "alonso de ovalle": "21280", "antonio varas": "21281",
    "arauco": "21282", "concepcion": "21283", "maipu": "21284", "melipilla": "21285",
    "nacimiento": "21286", "plaza norte": "21287", "plaza oeste": "21288",
    "plaza vespucio": "21289", "puente alto": "21290", "puerto montt": "21291",
    "san bernardo": "21293", "san carlos": "21294", "san joaquin": "21295",
    "valparaiso": "21296", "villarrica": "21297", "vina del mar": "16548",
}

_MESES_ES = ["", "enero", "febrero", "marzo", "abril", "mayo", "junio", "julio",
             "agosto", "septiembre", "octubre", "noviembre", "diciembre"]
_STOP_TALLER = {
    "taller", "talleres", "curso", "cursos", "clase", "clases", "evento", "eventos",
    "sobre", "de", "del", "la", "el", "los", "las", "un", "una", "para", "como",
    "quiero", "aprender", "hay", "algun", "alguno", "tema", "sesion", "proximo",
    "proxima", "que", "sea", "acerca",
}

_talleres_cache = {"ts": 0, "data": {}}  # cache simple: cid -> lista de eventos
...
def _parse_ical(texto):
    texto = _ical_unfold(texto)
    eventos = []
    for bloque in re.findall(r"BEGIN:VEVENT(.*?)END:VEVENT", texto, re.DOTALL):
        ev = {}
        for linea in bloque.strip().split("\n"):
            linea = linea.strip()
            if linea.startswith("SUMMARY:"):
                ev["titulo"] = _ical_des(linea[8:])
            elif linea.startswith("DESCRIPTION:"):
                ev["desc"] = _ical_des(linea[12:])
            elif linea.startswith("DTSTART"):
                m = re.search(r":(\d{8}T\d{6})", linea)
                if m:
                    ev["inicio"] = m.group(1)
            elif linea.startswith("URL:"):
                ev["url"] = linea[4:].strip()
            elif linea.startswith("LOCATION:"):
                ev["lugar"] = _ical_des(linea[9:])
            elif linea.startswith("CATEGORIES:"):
                ev["categoria"] = _ical_des(linea[11:])
        if ev.get("titulo") and ev.get("inicio"):
            eventos.append(ev)
    return eventos


async def _traer_ical(cid):
    """Descarga y parsea un calendario. Devuelve lista de eventos (o [])."""
    url = _ICAL_BASE + cid
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.get(url, headers={"User-Agent": "Mozilla/5.0 (chatbot-bibliotecas)"})
            if r.status_code != 200:
                return []
            return _parse_ical(r.text)
    except Exception as e:
        log.warning("ical %s fallo: %s", cid, e)
        return []


def _fecha_taller(utc):
    """'20260722T200000' (UTC) -> '22 de julio, 16:00 hrs' (hora Chile, UTC-4)."""
    try:
        dt = datetime.datetime.strptime(utc, "%Y%m%dT%H%M%S") - datetime.timedelta(hours=4)
        return f"{dt.day} de {_MESES_ES[dt.month]}, {dt.strftime('%H:%M')} hrs"
    except Exception:
        return ""


def _taller_futuro(ev):
    try:
        dt = datetime.datetime.strptime(ev["inicio"], "%Y%m%dT%H%M%S")
        return dt >= datetime.datetime.utcnow() - datetime.timedelta(hours=6)
    except Exception:
        return True


def _taller_coincide(ev, keywords):
    if not keywords:
        return True
    blob = _norm_basico(ev.get("titulo", "") + " " + ev.get("desc", "") + " " + ev.get("categoria", ""))
    return any(k in blob for k in keywords)


async def buscar_talleres(tema="", sede=""):
    """Devuelve talleres próximos: los del calendario central (online, para todos)
    y, si se indica sede, también los de esa sede. Filtra por tema si se da."""
    keywords = [k for k in _norm_basico(tema).split() if len(k) >= 3 and k not in _STOP_TALLER]

    # cache de 15 min por calendario (el feed declara TTL 15 min)
    ahora = time.time()
    if ahora - _talleres_cache["ts"] > 900:
        _talleres_cache["data"] = {}
        _talleres_cache["ts"] = ahora

    async def _cal(cid):
        if cid not in _talleres_cache["data"]:
            _talleres_cache["data"][cid] = await _traer_ical(cid)
        return _talleres_cache["data"][cid]

    resultado = {"online": [], "sede": [], "sede_nombre": None}

    # 1) central online (siempre)
    centrales = await _cal(CAL_CENTRAL_CID)
    resultado["online"] = [e for e in centrales if _taller_futuro(e) and _taller_coincide(e, keywords)]

    # 2) sede (si se indicó y existe)
    if sede:
        datos_sede = buscar_contacto_sede(sede)
        clave = None
        if datos_sede:
            # mapear el nombre normalizado de la sede a su clave de TALLERES_SEDES_CID
            n = _norm_basico(datos_sede["nombre"])
            for k in TALLERES_SEDES_CID:
                if k in n or n in k:
                    clave = k
                    break
        if not clave:
            sn = _ALIAS_SEDES.get(_norm_basico(sede), _norm_basico(sede))
            if sn in TALLERES_SEDES_CID:
                clave = sn
        if clave:
            resultado["sede_nombre"] = clave.title()
            evs = await _cal(TALLERES_SEDES_CID[clave])
            resultado["sede"] = [e for e in evs if _taller_futuro(e) and _taller_coincide(e, keywords)]

    # ordenar por fecha y limitar
    for k in ("online", "sede"):
        resultado[k] = sorted(resultado[k], key=lambda e: e["inicio"])[:5]
    return resultado


def _fmt_taller(ev):
    modalidad = ev.get("lugar") or ""
    extra = f" ({modalidad})" if modalidad else ""
    return {
        "titulo": ev.get("titulo"),
        "cuando": _fecha_taller(ev.get("inicio", "")),
        "modalidad": modalidad or None,
        "inscripcion": ev.get("url"),
    }


# ============================================================
# SALAS DE ESTUDIO (LibCal Spaces API — disponibilidad en vivo)
# ============================================================
# La reserva se hace en el portal con login del estudiante; el agente solo
# LEE disponibilidad (scope "Espacio - Leer") y entrega el enlace para reservar.
LIBCAL_BASE = "https://agenda-bibliotecas.duoc.cl"
LIBCAL_CLIENT_ID = os.environ.get("LIBCAL_CLIENT_ID", "")
LIBCAL_CLIENT_SECRET = os.environ.get("LIBCAL_CLIENT_SECRET", "")
# Página pública donde el estudiante reserva (con su cuenta Duoc)
URL_RESERVA_SALAS = "https://bibliotecas.duoc.cl/reserva-sala"
# Cada sede tiene su propia página de reserva de espacios en LibCal, con un
# "slug" corto. La misma página incluye salas de estudio, CIE y lentes VR
# (se elige con el selector de Categoría). Slugs VERIFICADOS desde el portal
# oficial (bibliotecas.duoc.cl/reserva-sala, jul 2026).
URL_RESERVA_BASE = "https://duoc.libcal.com/reserve/spaces/"
SALAS_SEDES_SLUG = {
    "alameda": "alameda", "antonio varas": "avaras", "arauco": "arauco",
    "concepcion": "concepcion", "maipu": "maipu", "melipilla": "melipilla",
    "nacimiento": "nacimiento", "alonso de ovalle": "povalle",
    "plaza norte": "pnorte", "plaza oeste": "poeste", "plaza vespucio": "pvespucio",
    "puente alto": "palto", "puerto montt": "pmontt", "san bernardo": "sbernardo",
    "san carlos": "scarlos", "san joaquin": "sjoaquin", "valparaiso": "valparaiso",
    "villarrica": "villarrica", "vina del mar": "vmar",
}

# Location ID de cada sede en LibCal (Admin > Espacios y Equipos)
SALAS_SEDES_LOC = {
    "alameda": 14008, "antonio varas": 15050, "arauco": 15059, "concepcion": 15096,
    "maipu": 15097, "melipilla": 15072, "nacimiento": 17910, "alonso de ovalle": 15100,
    "plaza norte": 15073, "plaza oeste": 15075, "plaza vespucio": 15065,
    "puente alto": 15098, "puerto montt": 15068, "san bernardo": 15099,
    "san carlos": 15139, "san joaquin": 15055, "valparaiso": 15061,
    "villarrica": 15047, "vina del mar": 15063,
}

_libcal_token = {"valor": None, "expira": 0}


async def _libcal_token_valido():
    """Devuelve un token OAuth vigente (lo renueva si expiró o falta poco)."""
    ahora = time.time()
    if _libcal_token["valor"] and ahora < _libcal_token["expira"] - 120:
        return _libcal_token["valor"]
    if not LIBCAL_CLIENT_ID or not LIBCAL_CLIENT_SECRET:
        log.warning("libcal SIN credenciales: id=%s secret=%s",
                    bool(LIBCAL_CLIENT_ID), bool(LIBCAL_CLIENT_SECRET))
        return None
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(
                f"{LIBCAL_BASE}/1.1/oauth/token",
                data={
            ...
            log.info("libcal oauth status=%s body=%s", r.status_code, r.text[:250])
            if r.status_code != 200:
                return None
            j = r.json()
            _libcal_token["valor"] = j.get("access_token")
            _libcal_token["expira"] = ahora + int(j.get("expires_in", 3600))
            log.info("libcal token OK, expira en %ss", j.get("expires_in"))
            return _libcal_token["valor"]
    except Exception as e:
        log.warning("libcal oauth EXCEPCION: %s", e)
        return None


async def _libcal_token_valido_OLD():
    """(reemplazada)"""
    ahora = time.time()
    if _libcal_token["valor"] and ahora < _libcal_token["expira"] - 120:
        return _libcal_token["valor"]
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            if r.status_code != 200:
                log.warning("libcal oauth %s: %s", r.status_code, r.text[:200])
                return None
            j = r.json()
            _libcal_token["valor"] = j.get("access_token")
            _libcal_token["expira"] = ahora + int(j.get("expires_in", 3600))
        ...
        log.warning("libcal oauth fallo: %s", e)
        return None


def _sede_a_locid(sede):
    datos = buscar_contacto_sede(sede)
    if datos:
        n = _norm_basico(datos["nombre"])
        for k, v in SALAS_SEDES_LOC.items():
            if k in n or n in k:
                return v, k.title(), k
    sn = _ALIAS_SEDES.get(_norm_basico(sede), _norm_basico(sede))
    if sn in SALAS_SEDES_LOC:
        return SALAS_SEDES_LOC[sn], sn.title(), sn
    return None, None, None


def _url_reserva_sede(clave):
    """URL de reserva específica de la sede (incluye salas y lentes VR).
    Si no hay slug para esa sede, cae a la página general."""
    slug = SALAS_SEDES_SLUG.get(clave)
    return (URL_RESERVA_BASE + slug) if slug else URL_RESERVA_SALAS


async def disponibilidad_salas(sede, fecha_iso, hora_desde=None):
    """Consulta salas de una sede en una fecha y devuelve cuántas están libres
    (opcionalmente desde cierta hora). fecha_iso = 'YYYY-MM-DD'.
    Retorna dict con estado y datos para que el modelo redacte."""
    locid, sede_nombre, sede_clave = _sede_a_locid(sede)
    if not locid:
        return {"estado": "sede_desconocida"}
    token = await _libcal_token_valido()
    if not token:
        return {"estado": "sin_api"}
    try:
        async with httpx.AsyncClient(timeout=20) as client:
            # 1) listar los espacios (salas) de la sede
            r = await client.get(
                f"{LIBCAL_BASE}/1.1/space/items/{locid}",
                params={"availability": fecha_iso},
                headers={"Authorization": f"Bearer {token}"},
            )
            if r.status_code != 200:
                log.warning("libcal items %s: %s", r.status_code, r.text[:200])
                return {"estado": "error_api"}
            items = r.json()
    except Exception as e:
        log.warning("libcal disponibilidad fallo: %s", e)
        return {"estado": "error_api"}
    ...
    ejemplos = []
    for it in items:
        # solo espacios reservables (no zonas/categorías)
        nombre = it.get("name") or ""
        avail = it.get("availability") or []
        total += 1
        # filtrar franjas por hora minima si se pidio
        libres = []
        for fr in avail:
            desde = fr.get("from", "")
            if hora_desde and desde:
                # 'from' viene como '2026-07-21T15:00:00-04:00'
                m = re.search(r"T(\d{2}):(\d{2})", desde)
                if m and f"{m.group(1)}:{m.group(2)}" < hora_desde:
                    continue
            libres.append(fr)
        if libres:
            con_cupo += 1
            if len(ejemplos) < 3:
                prim = libres[0].get("from", "")
                hm = re.search(r"T(\d{2}:\d{2})", prim)
                ejemplos.append({"sala": nombre, "desde": hm.group(1) if hm else None})
    return {
        "estado": "ok",
        "sede": sede_nombre,
        "fecha": fecha_iso,
        "salas_totales": total,
        "salas_con_cupo": con_cupo,
        "ejemplos": ejemplos,
        "url_reserva": _url_reserva_sede(sede_clave),
    }
...
TOOLS_CHATBOT = [
    ...
    {
        "name": "disponibilidad_salas",
        "description": (
            "Consulta EN VIVO si hay salas de estudio disponibles en una sede de Bibliotecas Duoc UC "
            "para una fecha y hora. Úsala cuando el estudiante quiera reservar o saber si hay salas "
            "libres. IMPORTANTE: primero pregúntale para cuándo la quiere (fecha y hora) y de qué sede, "
            "si no lo ha dicho. La reserva final la hace el estudiante en el portal con su cuenta Duoc; "
            "esta herramienta solo informa disponibilidad y entrega el enlace de reserva."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "sede": {
                    "type": "string",
                    "description": "Sede del estudiante (ej: 'Maipú', 'Antonio Varas')",
                },
                "fecha_iso": {
                    "type": "string",
                    "description": "Fecha deseada en formato AAAA-MM-DD (ej: '2026-07-21'). Si el estudiante dice 'hoy' o 'mañana', conviértelo tú a la fecha real.",
                },
                "hora_desde": {
                    "type": "string",
                    "description": "Hora mínima deseada en formato HH:MM (24h), opcional. Ej: '15:00'.",
                },
            },
            "required": ["sede", "fecha_iso"],
        },
    },
    {
        "name": "buscar_taller",
        "description": (
            "Busca talleres y eventos PRÓXIMOS de Bibliotecas Duoc UC en la agenda oficial y "
            "devuelve las sesiones con su fecha, hora, modalidad y enlace directo de inscripción. "
            "Incluye SIEMPRE los talleres online centralizados (abiertos a cualquier estudiante) y, "
            "si el estudiante indica su sede, también los de esa sede. Úsala cuando el estudiante "
            "pregunte por talleres/eventos o por aprender un tema que solemos cubrir (Excel, Word, "
            "PowerPoint, Canva, IA, prompts, búsqueda de información, APA, LinkedIn, competencias "
            "digitales, etc.). Puedes ofrecer talleres de forma proactiva."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "tema": {
                    "type": "string",
                    "description": "Tema o palabra clave del taller (ej: 'canva', 'excel', 'ia', 'apa'). Vacío = todos los próximos.",
                },
                "sede": {
                    "type": "string",
                    "description": "Sede del estudiante si la indicó (ej: 'Antonio Varas'). Opcional; sin sede solo muestra los online centralizados.",
                },
            },
        },
    },
...
async def ejecutar_herramienta(nombre, args):
    ...
    try:
        if nombre == "buscar_libro":
            ...
            salida = []
            for d in detalles[:5]:
                enlace_dig = d.get("enlace_digital") or None
                copias_fis = d.get("copias_disponibles", 0)
                item = {
                    "titulo": d.get("titulo"),
                    "autor": d.get("autor"),
                    "catkey": d.get("catkey"),
                    "anio_edicion": d.get("anio_edicion") or None,
                    "enlace_digital": enlace_dig,
                    "tiene_version_digital": bool(enlace_dig),
                    "copias_fisicas_disponibles": copias_fis,
                }
                salida.append(item)
            instruccion = ("IMPORTANTE: ofrece SOLO los formatos que existen. Para cada obra, "
                           "'tiene_version_digital' indica si hay versión digital (si es false, NO "
                           "ofrezcas digital, NO preguntes '¿la quieres digital?'). Si hay versión "
                           "digital, puedes entregar el enlace. Para saber la disponibilidad física "
                           "por sede usa ver_disponibilidad con el catkey. NUNCA ofrezcas un formato "
                           "que no aparezca como disponible aquí: genera expectativas falsas.")
            return json.dumps({"instruccion": instruccion, "obras": salida}, ensure_ascii=False)
        ...
        if nombre == "disponibilidad_salas":
            sede = str(args.get("sede", ""))[:60].strip()
            fecha = str(args.get("fecha_iso", ""))[:10].strip()
            hora = str(args.get("hora_desde", "")).strip() or None
            if not re.match(r"^\d{4}-\d{2}-\d{2}$", fecha):
                return ("Falta la fecha en formato AAAA-MM-DD. Pregúntale al estudiante para qué "
                        "día quiere la sala (hoy es " + _hoy_chile() + ").")
            r = await disponibilidad_salas(sede, fecha, hora)
            est = r.get("estado")
            if est == "sede_desconocida":
                return "No reconozco esa sede. Pídele al estudiante que indique en qué sede quiere la sala."
            if est == "sin_api":
                return ("La consulta de salas en vivo no está disponible ahora. Ofrece en positivo el "
                        f"enlace para reservar directamente con su cuenta Duoc: {URL_RESERVA_SALAS}")
            if est != "ok":
                return ("No se pudo consultar la disponibilidad en este momento. Ofrece en positivo el "
                        f"enlace de reserva: {URL_RESERVA_SALAS}")
            return json.dumps({
                "instruccion": ("Informa la disponibilidad en tono positivo y SIEMPRE entrega el enlace "
                                "de reserva en markdown [Reserva tu sala aquí](url) usando el 'url_reserva' "
                                "de abajo (es la página específica de ESA sede), recordando que la reserva "
                                "se hace con su cuenta Duoc. En esa misma página, con el selector de "
                                "Categoría, también puede reservar lentes VR y otros espacios (menciónalo "
                                "brevemente si viene al caso). Si hay salas con cupo, invítalo a reservar; "
                                "si no hay a esa hora, sugiere otro horario o el enlace para revisar."),
                **r,
            }, ensure_ascii=False)

        if nombre == "buscar_taller":
            tema = str(args.get("tema", ""))[:60].strip()
            sede = str(args.get("sede", ""))[:60].strip()
            r = await buscar_talleres(tema=tema, sede=sede)
            online = [_fmt_taller(e) for e in r["online"]]
            desede = [_fmt_taller(e) for e in r["sede"]]
            if not online and not desede:
                base = ("No hay talleres próximos que coincidan con esa búsqueda en este momento. "
                        "Invita al estudiante a revisar la agenda completa por si se agregan nuevos: "
                        "https://agenda-bibliotecas.duoc.cl/calendar/talleres . NO inventes talleres. "
                        "IMPORTANTE: NO inventes correos ni teléfonos de contacto. Si el estudiante "
                        "quiere contactar a su biblioteca, usa la herramienta contacto_biblioteca "
                        "(NUNCA escribas un correo por tu cuenta como 'valparaiso@duoc.cl' o similar). "
                        "NO recomiendes bases de datos específicas (O'Reilly, eLibro, etc.); si acaso, "
                        "ofrece una búsqueda del tema en el Descubridor.")
                return base
            return json.dumps({
                "instruccion": ("Presenta estos talleres REALES con su fecha, hora y enlace de "
                                "inscripción en markdown [Inscríbete aquí](url). Destaca los 'online' "
                                "como abiertos a todos sin importar la sede. Si hay de la sede del "
                                "estudiante, muéstralos aparte. No inventes ni agregues talleres que no estén aquí."),
                "talleres_online_para_todos": online,
                "talleres_de_su_sede": desede,
                "sede_consultada": r.get("sede_nombre"),
            }, ensure_ascii=False)
        ...
        if nombre == "contacto_biblioteca":
            ...
            if not datos:
                ...
                return (f"Sede no reconocida. Pide al estudiante que precise cuál de estas: {sedes}. "
                        "También puedes ofrecer el canal general: el formulario "
                        "https://bibliotecas.duoc.cl/consultanos (que seleccione su sede)")
            info = {
            ...
            tiene_correo = bool(datos.get("correo"))
            tiene_jefe = bool(datos.get("jefe"))
            if tiene_correo:
                # Caso normal: PRIVILEGIAR el correo genérico de la biblioteca.
                instruccion = (
                    "INSTRUCCION: entrega SIEMPRE como opción principal el CORREO GENÉRICO de la "
                    "biblioteca (campo correo_biblioteca). Ese es el canal preferente para casi todo. "
                    + ("Solo si el estudiante lo pide explícitamente, o si es un caso puntual y más "
                       "grave que amerite escalar, puedes OFRECER ADEMÁS contactar al jefe/a de "
                       "biblioteca como segunda opción. No pongas al jefe por delante del correo "
                       "genérico ni lo entregues por defecto. " if tiene_jefe else "")
                    + "No muestres el equipo completo salvo que lo pidan.")
            elif tiene_jefe:
                # Sedes sin correo genérico (ej. Viña del Mar, Puerto Montt): el jefe es la vía.
                instruccion = (
                    "INSTRUCCION: esta sede no tiene correo genérico registrado, así que la vía de "
                    "contacto por correo es el jefe/a de biblioteca (entrégalo). Ofrece también el "
                    "teléfono y el formulario https://bibliotecas.duoc.cl/consultanos (recordando "
                    "seleccionar la sede). No muestres el equipo completo salvo que lo pidan.")
            else:
                # Campus sin jefe y sin correo genérico
                instruccion = (
                    "INSTRUCCION: entrega el correo genérico de la biblioteca (y fono si tiene) sin "
                    "preguntar. Si no hubiera correo, ofrece el teléfono y el formulario "
                    "https://bibliotecas.duoc.cl/consultanos. No muestres el equipo salvo que lo pidan.")
...
def _build_payload(historial, stream):
    ...
    # Bloque 1: el prompt grande, CACHEADO (no cambia entre peticiones).
    # Bloque 2: la fecha de HOY, fresca en cada petición y SIN cachear (es
    # minúsculo). Así el día de la semana siempre es correcto y el caché del
    # prompt grande se mantiene. El modelo NUNCA debe deducir el día solo.
    fecha_hoy = _hoy_chile_texto()
    system_blocks = [
        {"type": "text", "text": SYSTEM_PROMPT, "cache_control": {"type": "ephemeral"}},
        {"type": "text", "text": (
            f"FECHA Y HORA ACTUAL (zona horaria de Chile): hoy es {fecha_hoy}. "
            "USA SIEMPRE esta fecha y este día de la semana; NUNCA los deduzcas ni "
            "los inventes. Cuando el estudiante diga 'hoy', 'mañana', 'este viernes', "
            "etc., calcula la fecha a partir de este dato. 'Mañana' es el día "
...
def health():
    ...
    return {
        ...
        "fecha_hoy_servidor": _hoy_chile_texto(),
        "metricas_sheets": bool(METRICAS_URL and METRICAS_TOKEN),
        "libcal": {
            "client_id_detectado": bool(LIBCAL_CLIENT_ID),
            "secret_detectado": bool(LIBCAL_CLIENT_SECRET),
            "salas_sedes": len(SALAS_SEDES_LOC),
        },
    }
...
async def chat(req: ChatRequest, request: Request):
    ...
    _stats["total"] += 1
    _consulta_texto = historial[-1]["content"] if historial else ""
    _conv_id = req.conversation_id or ""
    ...
    if faq is not None:
        ...
        asyncio.create_task(_registrar_metrica(_consulta_texto, "faq", _conv_id, faq))
    ...
    _modelo_metrica = "haiku" if "haiku" in modelo else "sonnet"
    ...
    async def event_stream():
        ...
        _respuesta_completa = ""  # acumula toda la respuesta para la métrica
        async with httpx.AsyncClient(timeout=120) as client:
            for _ronda in range(4):
                ...
                if stop_reason != "tool_use" or not tool_uses:
                    # respuesta final entregada: registrar métrica con consulta + respuesta
                    asyncio.create_task(_registrar_metrica(
                        _consulta_texto, _modelo_metrica, _conv_id, _respuesta_completa))
                ...
                for tu in tool_uses:
                    ...
                    res = await ejecutar_herramienta(tu["name"], args)
            ...
            asyncio.create_task(_registrar_metrica(
                _consulta_texto, _modelo_metrica, _conv_id,
                _respuesta_completa or "[sin respuesta: se agotaron las rondas]"))

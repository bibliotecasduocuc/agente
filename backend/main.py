...
- Reservar sala de estudio: tienes la herramienta disponibilidad_salas para consultar EN VIVO si hay salas libres en una sede para una fecha/hora. FLUJO: cuando el estudiante quiera una sala, pregúntale primero PARA CUÁNDO (fecha y hora) y de QUÉ SEDE, si no lo dijo. Convierte "hoy"/"mañana" a fecha real (hoy es """ + _hoy_chile() + """, zona Chile). Luego llama la herramienta y comunica la disponibilidad, entregando SIEMPRE el enlace para reservar con su cuenta Duoc: https://bibliotecas.duoc.cl/reserva-sala — la reserva final la hace el estudiante ahí (con login); tú informas y enlazas. Videotutorial de reserva: https://www.youtube.com/watch?v=SxU_2BFHVI4
- Reservar lentes VR: https://bibliotecas.duoc.cl/reserva-sala (misma página de reservas)
...
def _sede_a_locid(sede):
    ...
    if sn in SALAS_SEDES_LOC:
        return SALAS_SEDES_LOC[sn], sn.title()
    return None, None
...
async def disponibilidad_salas(sede, fecha_iso, hora_desde=None):
    ...
    locid, sede_nombre = _sede_a_locid(sede)
    if not locid:
        ...
        if nombre == "disponibilidad_salas":
            ...
            return json.dumps({
                "instruccion": ("Informa la disponibilidad en tono positivo y SIEMPRE entrega el enlace "
                                "de reserva en markdown [Reserva tu sala aquí](url), recordando que la "
                                "reserva se hace con su cuenta Duoc. Si hay salas con cupo, invítalo a "
                                "reservar; si no hay a esa hora, sugiere otro horario o el enlace para revisar."),
                **r,
            }, ensure_ascii=False)
